...
_ARTIFACTS_PATH = os.path.join(os.path.dirname(__file__), "ids_model")
...
def predict_connection(input_dict: dict):
    ...
    # --- Decision Tree ---
    # BUG FIX: Only include numeric/feature values, not keys, and append dummy label
    dt_input_row = [input_dict[col] for col in _numeric_cols]
    dt_input_row.append(0)  # dummy label placeholder required by _classify
    ...
    # BUG FIX: Align column names so XGBoost doesn't raise feature name mismatch warning/error
